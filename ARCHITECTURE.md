...
The **full vertical slice** (migrations → domain → repository → service → HTTP module → router → `app.go` → Swagger) is documented in the next section — **mirrored from [README.md](README.md)** so architecture and onboarding stay in sync.
...
- [ ] Add a row to the **“Every file explained”** table in **[README.md](README.md)** for your new files (keeps onboarding accurate).

*(Same content as README § “How to add a new feature module”; update both places if you change the recipe.)*
